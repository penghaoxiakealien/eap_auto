...
head=10
top_k=1
typename="negative_name_mover_head"
suffix="_l"

# 拼接输出目录
output_dir="$(dirname "$0")/../../results/ioi/hypothesis/${layer}.${head}${suffix}"
...
for round in {1..5}; do
    ...
    CUDA_VISIBLE_DEVICES=6 python auto_logit_accuracy.py --layer $layer --head $head --rounds $round --typename $typename --output_dir "$output_dir"

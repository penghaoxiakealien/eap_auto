...
if [[ -z "$RECEIVER_DESC_FILES" ]]; then
    echo "错误: --receiver-desc 为必填参数。请先提供下游 SIH/DTH 或目标 head 的 best_hypothesis.json 映射。" >&2
    usage
    exit 1
fi
...
if [[ -n "$TARGET_HEADS" ]]; then
    ...
    TARGET_LABEL="${TARGET_LABEL//,/__}"
else
    TARGET_LABEL="${TARGET_HEAD//./_}"
fi

if [[ -n "$DATA_DIR_OVERRIDE" ]]; then
    DATA_SOURCE_DIR="$DATA_DIR_OVERRIDE"
else
    DATA_SOURCE_DIR="$BASE_RESULTS_DIR/path_patching/Middle_Head_Plus/${LAYER}_${HEAD}_to_${TARGET_LABEL}"
fi
if [[ -n "$OUTPUT_DIR_OVERRIDE" ]]; then
    OUTPUT_DIR="$OUTPUT_DIR_OVERRIDE"
else
    OUTPUT_DIR="$BASE_RESULTS_DIR/hypothesis/Middle_Head_Plus/${LAYER}.${HEAD}_to_${TARGET_LABEL}_single_${CAUSAL_DIRECTION}_$(date +%Y%m%d_%H%M)"
fi
...
MIDDLE_DIFF_FILE="$DATA_SOURCE_DIR/causal_dataset_${LAYER}_${HEAD}_to_${TARGET_LABEL}.json"
...
echo "=== Step 4: export attention_scores_ground_truth.jsonl for auto_single_middle ==="
...
for pair in pairs:
    ...
    try:
        ...
        if isinstance(data, dict) and "best_hypothesis" in data:
            mapping[head] = data.get("best_hypothesis", "")
            continue
        candidates = data.get("all_hypotheses_validation_results") if isinstance(data, dict) else None
        if not candidates:
            continue
        best = max(candidates, key=lambda item: (item.get("validation_scores", {}).get("direct_attention_f1", 0.0), item.get("validation_scores", {}).get("causal_f1", 0.0)))
        mapping[head] = best.get("hypothesis", "")
...
echo "=== Step 5: compute sender→target diffs via intermediates ==="
...
if [[ -n "$TARGET_HEADS" ]]; then
    RECEIVER_HEADS="$TARGET_HEADS"
else
    RECEIVER_HEADS="$TARGET_HEAD"
fi

echo "=== Step 6: run auto_single_middle.py for ${head_str} (target ${RECEIVER_HEADS}, direction ${CAUSAL_DIRECTION}) ==="
python "$REPO_ROOT/tests/experiments/auto_single_middle.py" \
    --layer "$LAYER" \
    --head "$HEAD" \
    --rounds "$ROUNDS" \
...
if [ -f "$TEST_RESULT_FILE" ]; then
    python "$REPO_ROOT/tests/experiments/save_final_hypothesis.py" \
        ...
        --head "$LAYER.$HEAD" \
        --typename "Middle_Head_Plus_Single"
else
    echo "⚠️ 未找到测试结果 $TEST_RESULT_FILE，跳过 final_hypothesis 生成。"
fi

echo "✅ Single-direction middle head plus pipeline finished. Results saved to $OUTPUT_DIR"

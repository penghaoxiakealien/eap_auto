...
def logits_to_avg_logit_diff(logits: t.Tensor, dataset: IOIDataset) -> t.Tensor:
...
def run_path_patching(
    ...
    hook_fn = lambda act, hook: patch_or_freeze_head_vectors(act, hook, abc_cache, ioi_cache, head)
...
def update_soft_payload(
    ...
    per_head = soft_data.get("patterns", {}).get("per_head") or soft_data.get("head_patterns")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对 dominant 聚类后的注意力头进行路径插补分类")
...
def run_path_patch_analysis(
    ...
    pattern_groups = soft_data.get("pattern_groups") or patterns_block.get("groups_by_signature") or {}
    ...
    z_filter = lambda name: name.endswith("z")
    with t.no_grad():
        _, abc_cache = model.run_with_cache(abc_dataset.toks, names_filter=z_filter, return_type=None)
        _, ioi_cache = model.run_with_cache(dataset.toks, names_filter=z_filter, return_type=None)

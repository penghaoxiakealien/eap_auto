      ...
      messages = [
            {
                  ...
                  "- Use this to infer what the head is actually doing and revise the hypothesis. A successful revision should explain both the attention patterns and the head's causal effect (contributory or inhibitory) as revealed by the logit contributions.\n"
                  "- Your revised hypothesis should reduce such mistakes to the greatest extent possible\n"
                  "- The hypothesis should describe the dominant functional behavior using precise linguistic, semantic, or structural terminology. Avoid overly abstract or generic phrasing like “tracks entities” or “maintains context”."
                  "- Analyze the examples by grouping them into input categories (e.g., sentence structures, entity numbers and roles, or context types) and explain how the head behaves differently across these types. Base your final hypothesis on this classification-aware reasoning."
                  "- Think of the 'Logit Prediction' as a check on your fundamental understanding. If your logit predictions are consistently wrong, it suggests the old hypothesis may have misinterpreted the head's basic role (e.g., viewing an inhibitory head as contributory). This is a key insight for refining your hypothesis.\n"
                  ...
                  "Your hypothesis should explain how these two aspects are connected. For instance, does a specific attention pattern lead to a contributory (positive) or inhibitory (negative) contribution, and why?\n\n"
...
async def main():
      ...
      precomputed_file = os.path.join(os.path.dirname(__file__), "..", "..", 
                              "results", "ioi", "path_patching", 
      ...
      path_patching_explanations = """
            ...
            **Crucial Interpretability Context:**
            You MUST interpret the results of this experiment correctly:
            - If patching causes performance to 'decrease' or 'get worse' (a negative logit difference), it means the head's original function is **contributory (aids the task)**.
            - If patching causes performance to 'increase' or 'get better' (a positive logit difference), it means the head's original function is **inhibitory (hinders the task)**.
            Your entire hypothesis must be consistent with this causal interpretation.
      """
      explanations = {
            "name_mover_head": f"Attention head {head} in layer {layer} has a high influence to the logit difference of the model output. By path-patching this head to the final logit difference of the model output with irrelavant inputs, the logit difference of indirect subject and subject decreased, causing the model to perform worse on IOI task.",
            "negative_name_mover_head": f"Attention head {head} in layer {layer} has a high influence to the logit difference of the model output. By path-patching this head to the final logit difference of the model output with irrelavant inputs, the logit difference of indirect subject and subject increased, causing the model to perform better on IOI task.",
      ...
      while True:
            ...
            # 2. 计算真实attention score
            example_attention_score_filename = f"model_prompt_attention_scores_{rounds}_{iteration}.jsonl"
            ...
            with open(example_attention_score_path, "r") as f:
            ...
            results.append({
                  ...
                  "scores": {
                        "accuracy": accu, "f1": f1, "ndcg": ndcg, "Kendall Tau": kendall_tau,
            ...
            print(f"Initial scores for Iteration {iteration}: Accuracy={accu:.2f}, F1={f1:.2f}, Kendall Tau={kendall_tau:.2f}, Logit Accuracy={logit_comparison['accuracy']:.2%}")

            # 8. 检查是否满足终止条件
            if (accu >= 0.95 and f1 >= 0.95 and kendall_tau >= 0.95 and logit_comparison["accuracy"] >= 0.95):
                print("Hypothesis is valid. All scores meet the criteria.")
                break
            ...
            # =================================================================
                ...
                # 准备反馈文本
                attention_feedback_text = extract_predicted_attention(predict_attention_json, real_attention_json)
                logit_feedback_text_list = []
                for key, pred_sign in logit_comparison["predicted"].items():
                    real_sign = logit_comparison["real"].get(key, "unknown")
                    status = "Right" if pred_sign == real_sign else "Wrong"
                    logit_feedback_text_list.append(f"Logit Prediction for {key}: {status} (Predicted: {pred_sign.capitalize()}, Real: {real_sign.capitalize()})")
                logit_feedback_text = "\n".join(logit_feedback_text_list)

                # --- 步骤一：仅基于注意力模式进行精炼 ---
                print("\n--- Step 1: Refining hypothesis based on ATTENTION PATTERNS only ---")
                hypothesis_text_step1 = await compare_and_refine_hypothesis(
                    open_router, 
                    explanation, 
                    extracted_hypothesis, 
                    attention_feedback_text, # 提供注意力反馈
                    "",                      # **关键：忽略Logit反馈**
                    layer, head, 
                    output_dir
                )
                _, hypothesis_step1 = extract_hypothesis_text(hypothesis_text_step1)
                if not hypothesis_step1:
                    print("Warning: Step 1 refinement failed to produce a new hypothesis. Using old one.")
                    hypothesis_step1 = extracted_hypothesis

                # --- 步骤二：基于第一步结果，仅基于Logit贡献进行精炼 ---
                print("\n--- Step 2: Refining hypothesis based on LOGIT CONTRIBUTIONS only ---")
                final_hypothesis_text = await compare_and_refine_hypothesis(
                    open_router,
                    explanation,
                    hypothesis_step1,        # **关键：使用第一步精炼后的假设**
                    "",                      # **关键：忽略注意力反馈**
                    logit_feedback_text,     # 提供Logit反馈
                    layer, head,
                    output_dir
                )
                
                # 更新假设以进行下一轮迭代
                hypothesis_analysis, extracted_hypothesis = extract_hypothesis_text(final_hypothesis_text)
                if not extracted_hypothesis:
                    print("Warning: Step 2 refinement failed. Re-using hypothesis from Step 1.")
                    extracted_hypothesis = hypothesis_step1

            if iteration >= iter:
                  print(f"Max iterations ({iteration}) reached. Stopping.")
                  break
            iteration += 1

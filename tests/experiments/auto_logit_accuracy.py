...
async def main():
      ...
      while True:
            ...
            # 打印新的准确率指标
            print(f"  Logit Prediction Accuracy: {logit_comparison['accuracy']:.2%}")

            # 将logit评估结果添加到results中
            results[-1]["scores"]["logit_accuracy"] = logit_comparison["accuracy"]
            ...
            # 修改终止条件，使用新的 logit_accuracy 指标
            if (accu >= 1.0 and f1 >= 1.0 and ndcg >= 0.95 and kendall_tau >= 0.99 
                and ndcg_pre >= 0.8 and kendall_tau_pre >= 0.8 and logit_comparison["accuracy"] >= 0.9):
                ...
                for key, pred_sign in logit_comparison["predicted"].items():
                    ...
                    logit_comparison_text.append(f"{key} (Predicted): {pred_sign.capitalize()}")
                    logit_comparison_text.append(f"{key} (Real): {real_sign.capitalize()}")

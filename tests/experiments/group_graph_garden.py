...
# -*- coding: utf-8 -*-
"""
将 garden NPZ v-trans (mod) 的 head 图按 dominant signature 聚合为“组图”（group graph）。

输入：
  - --graph-json: head 级图（nodes/edge_list），例如 filtered/graph_thr0_001.json
  - --dominant-soft: extract_dominant_patterns_garden.py 输出的 *_soft.json
...
聚合规则：
  ...
  - （可选）若提供 --layer-buckets，则在 signature 基础上再按 layer bucket 细分组，
    组 id 变为 "{signature}::L{bucket_label}"，例如 "SUBJ:SUBJ->OBJ_HEAD@+0::L0-4"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build garden grouped graph from dominant signatures.")

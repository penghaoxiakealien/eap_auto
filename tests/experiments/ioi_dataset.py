...
class IOIDataset:
    ...
    def gen_swapped_s_io_prompts(self):
        """
        【新功能】生成一个对照数据集，其中每个句子的S和IO在第一次出现时的位置被交换。
        例如: "Mary and John ... Mary gave..." -> "John and Mary ... Mary gave..."
        这与 gen_flipped_prompts 不同，后者通常用随机名字替换。
        """
        if self.has_been_flipped:
            warnings.warn(
                "This dataset appears to have been modified already. Chaining modifications can lead to unexpected results."
            )

        # 1. 创建原始prompts的深拷贝，以避免修改原始对象
        swapped_prompts = copy.deepcopy(self.ioi_prompts)
        ...
        for prompt in swapped_prompts:
            ...
            # 3. 执行一个安全的、仅替换第一次出现的三步交换
            # a. 将第一个 S 替换为临时占位符
            temp_text = original_text.replace(s_token, "___TEMP_S___", 1)
            # b. 将第一个 IO 替换为 S
            temp_text = temp_text.replace(io_token, s_token, 1)
            # c. 将临时占位符替换为 IO
            new_text = temp_text.replace("___TEMP_S___", io_token, 1)
            
            prompt["text"] = new_text
            
            # 4. 【重要】更新prompt字典中的S和IO标签，因为它们现在互换了角色
            prompt["S"] = io_token
            prompt["IO"] = s_token

        # 5. 使用修改后的prompts创建一个新的IOIDataset实例
        swapped_dataset = IOIDataset(
            prompt_type=self.prompt_type,
            N=self.N,
            tokenizer=self.tokenizer,
            ...
            prefixes=self.prefixes,
            prepend_bos=self.prepend_bos,
            manual_word_idx=self.word_idx, # 复用原始的索引，因为结构没变
            has_been_flipped=True, # 标记为已修改
            seed=self.seed + 1, # 使用不同的seed以示区别
            device=self.device,
        )
        return swapped_dataset
    ...
    def generate_dataset_until_last_to(self):
        """
        Generate a new dataset where prompts are truncated up to the last occurrence of 'to'.

        Returns:
            IOIDataset: A new dataset with truncated prompts.
        """
        # Create a copy of the current dataset
        truncated_prompts = copy.deepcopy(self.ioi_prompts)

        # Truncate each prompt's text up to the last occurrence of "to"
        for prompt in truncated_prompts:
            text = prompt["text"]
            last_to_idx = text.rfind(" to ")
            if last_to_idx != -1:
                prompt["text"] = text[:last_to_idx + 4]  # Include " to "

        # Create a new dataset with the truncated prompts
        truncated_dataset = IOIDataset(
            prompt_type=self.prompt_type,
            N=len(truncated_prompts),
            tokenizer=self.tokenizer,

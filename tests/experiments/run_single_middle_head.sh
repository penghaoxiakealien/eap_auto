...
usage() {
    cat <<'EOF'
用法: run_single_middle_head.sh [选项]
  --layer L                 发送头所在层
  --head H                  发送头编号
...
if [[ -n "$OUTPUT_DIR_OVERRIDE" ]]; then
    ...
    OUTPUT_DIR="$BASE_RESULTS_DIR/hypothesis/Middle_Head/${LAYER}.${HEAD}_single_${CAUSAL_DIRECTION}_$(date +%Y%m%d_%H%M)"
fi
...
echo "=== Step 6: run auto_single_middle.py for ${head_str} (direction ${CAUSAL_DIRECTION}) ==="
...
if [ -f "$TEST_RESULT_FILE" ]; then
    python "$REPO_ROOT/tests/experiments/save_final_hypothesis.py" \
        ...
        --head "$LAYER.$HEAD" \
        --typename "Middle_Head_Single"
else
    echo "⚠️ 未找到测试结果 $TEST_RESULT_FILE，跳过 final_hypothesis 生成。"
fi

echo "✅ Single-direction middle head pipeline finished. Results saved to $OUTPUT_DIR"

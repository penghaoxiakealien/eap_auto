...
def load_edge_specs(
    ...
    for src, dst in edges:
        ...
        if dst == "logits":
            ...
            if joint_patch and len(channels) > 1:
                specs.append(EdgeSpec(
                    src=HeadRef.from_name(src),
                    dst=HeadRef.from_name(dst),
                    target_kind="attention",
...
def patch_or_freeze_head_vectors(
    ...
    # 先恢复 clean，再替换一个 sender 头为 corrupted
...
def path_patch_input_to_head_joint(
    ...
    """
    sender = input（resid_pre@layer0 用 corrupted 替换 clean）
    receiver = 某个 head 的 q/k/v 联合（将该 head 的 q/k/v 输入替换为 sender-patched run 缓存）
    返回单个 delta 值（mean over prompts）。
    """
    receiver_layer = receiver_head.layer
    recv_hook_names = {utils.get_act_name(ch, receiver_layer) for ch in receiver_channels}
    recv_filter = lambda name: name in recv_hook_names

    # input 节点：使用 blocks.0.hook_resid_pre
    ...
    # 先缓存 sender=input 被替换后的 receiver 输入
    with t.no_grad():
    ...
    # 再在 clean run 中注入 receiver 输入（不再 patch input）
    def mk_recv_hook(hname: str):
        def fn(tensor, hook):
            if hook.name == hname:
                tensor[:, :, receiver_head.head] = recv_cache[hname][:, :, receiver_head.head]
...
def run_edge_path_patching(
    ...
    specs = load_edge_specs(collapsed_graph, channels, joint_patch)
    if not specs:
...
# --------------- CLI --------------- #
def parse_args():
    p = argparse.ArgumentParser(description="Path patching for agr_gender (q/k/v + optional qkv)")
    p.add_argument("--collapsed-graph", type=Path, required=True)
    p.add_argument("--output", type=Path, required=True)
    p.add_argument("--data-path", type=Path, required=True, help="agr_gender 数据文件")

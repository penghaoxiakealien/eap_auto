...
# -*- coding: utf-8 -*-
"""
从 classify_agr_gender.py 的输出中抽取“主导模式”（dominant pattern）。

定义（与 IOI 流水线类比）：
- 对每个 head、每个 query 位置（如 END/VERB/A1/B/A2），取该位置 top1 pattern 的 ratio=count/total。
- 若某个位置的 ratio >= threshold，则认为该 head 在该位置存在主导模式；
  选取 ratio 最大的那个位置/模式作为 head 的 dominant。
- 若所有位置都 < threshold，则标为 pending，仍选 ratio 最大的 top1 作为“最强候选”。

输出：
1) <output_prefix>_thrXX.json：包含 dominant_assignments（每个 head 的 dominant 信息）
2) <output_prefix>_thrXX_soft.json：与原 patterns-json 同结构，但将 per_head.signature 更新为 dominant signature，
   并重建 groups_by_signature，便于 visualize_agr_gender.py 直接标注。
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Extract dominant patterns for agr_gender classify output.")
    p.add_argument("--patterns-json", type=Path, required=True, help="classify_agr_gender 输出 JSON")
...
def build_soft(base: Dict[str, Any], assignments: Dict[str, Any]) -> Dict[str, Any]:
    out = json.loads(json.dumps(base))  # deep copy without importing copy
...
def main() -> None:
    ...
    if not isinstance(per_head, dict):
        raise ValueError("patterns-json 缺少 per_head dict（请确认输入来自 classify_agr_gender.py）")

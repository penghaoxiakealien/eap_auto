...
def calculate_logit_contribution(layer, head, example_sentences, 
    ...
    if head_key in head_contributions:
        # 原始值是百分比形式，需要转换为0-1范围
        raw_contribution = head_contributions[head_key] / 100.0
        # 取绝对值并归一化到0-1范围
        base_contribution = min(1.0, abs(raw_contribution))
    else:
        print(f"注意力头 {head_key} 的贡献度数据不存在，使用默认值0.0")
    ...
    for key, sentence_data in example_sentences.items():
        ...
        entity_count = sentence.count(" and ") + 1  # 简单估算实体数量
        
        # 长句子和多实体的情况下，贡献度可能略有不同
        if sentence_length > 20:
            contribution *= 1.1  # 稍微增加
        if entity_count > 2:
            contribution *= 1.05
    ...
    print(f"使用预计算贡献度 - 头 {layer}.{head}: 基础贡献度 = {base_contribution:.4f}")

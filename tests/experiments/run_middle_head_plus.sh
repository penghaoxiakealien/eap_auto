...
if [[ -n "$OUTPUT_DIR_OVERRIDE" ]]; then
    ...
    OUTPUT_DIR="$BASE_RESULTS_DIR/hypothesis/Middle_Head_Plus/${LAYER}.${HEAD}_to_${TARGET_LABEL}_$(date +%Y%m%d_%H%M)"
fi
...
#############################################
# Step 3: build preprocessed_attention_scores.json
#############################################
...
#############################################
# Step 4: 导出 attention_scores_ground_truth
#############################################
...
echo "=== Step 6: run auto_s_att.py for ${head_str} (target ${RECEIVER_HEADS}) ==="
...
if [ -f "$TEST_RESULT_FILE" ]; then
    python "$REPO_ROOT/tests/experiments/save_final_hypothesis.py" \
        ...
        --head "$LAYER.$HEAD" \
        --typename "Middle_Head_Plus"
else
    echo "⚠️ 未找到测试结果 $TEST_RESULT_FILE，跳过 final_hypothesis 生成。"
fi

echo "✅ Middle head plus pipeline finished. Results saved to $OUTPUT_DIR"

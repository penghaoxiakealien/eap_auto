...
def get_direct_attention_examples_for_sampling(sampled_sentences, attention_ground_truth, top_k=2):
    ...
    for sid, data in sampled_sentences.items():
        ...
        raw_tokens = gt.get("tokens_suffixed") or gt.get("tokens", [])
        scores = gt.get("scores", [])
        top_tokens = []
        for i, tok in enumerate(raw_tokens[:top_k]):
            sc = scores[i] if i < len(scores) else 0.0
            top_tokens.append((tok, float(sc)))
        examples.append({
...
async def predict_for_sentence_batch(open_router, hypothesis, sentences_data, sender_head, top_k, output_dir, receiver_attention_position, hop_info, max_retries=1, strict_mode=False):
    """
    ...
    for sid, data in sentences_data.items():
        ...
        if sentence_text:
            display_sid = format_garden_sid(sid)
            sentence_lines.append(f"`{display_sid}: {sentence_text}`")
    if not sentence_lines:
        return {}

    system_prompt = (
        "You are applying a hypothesis to predict experimental outcomes in a Garden Path NP/Z v-trans task. "
        "Your task is to predict how corrupting a Sender Head changes the attention of downstream receiver heads.\n\n"
        "**--- Key Concepts in the Garden Task (Crucial Background) ---**\n"
        "The Garden task is about correctly predicting the **disambiguation token** at the sentence end (\"was\" or \"for\").\n"
        "Important positions:\n"
        "- **SUBORD:** clause introducer (As/While/After/When)\n"
        ...
        f"{hop_info}\n\n"
        "--- **Core Task & Causal Rules** ---\n"
        f"Predict how corrupting Sender Head {sender_head} changes downstream attention.\n"
        "Mark tokens whose downstream attention INCREASES with `<<token>>`.\n"
        "Mark tokens whose downstream attention DECREASES with `[[token]]`.\n\n"
        "**MUST FOLLOW RULES:**\n"
        "- A token cannot be marked as both increase and decrease.\n"
        "- You must highlight exactly one token for increase and one token for decrease.\n"
        "- Do not change the sentence text apart from inserting the markers.\n"
        "- Return all outputs in a single `[PREDICTION]` block, one line per sentence, preserving the sid prefix.\n"
    )
    user_prompt = (
        f"**Hypothesis:** {hypothesis}\n\n"
    ...
    for sid, data in sentences_data.items():
        tasks.append(
            predict_for_single_sentence(
                open_router, hypothesis, sid, data, sender_head, top_k, output_dir, receiver_attention_position, hop_info, False
            )
        )
...
async def predict_attention_changes_for_sentences(open_router, hypothesis, sentences_data, sender_head, top_k, output_dir, receiver_attention_position, hop_info, with_reasoning):
    """
    让LLM根据假设，为一批句子按每批5句预测注意力变化。
    """
    print("正在根据假设按每批5句预测注意力变化...")
    predictions = {}
    for batch in chunk_dict(sentences_data, 5):
        batch_predictions = await predict_for_sentence_batch(
            open_router,
            hypothesis,
            batch,
            sender_head,
...
async def main():
    ...
    for candidate_idx in range(1, 4):
        ...
        if optimize_only != "causal" and not direct_attention_examples:
            print(f"无法为初始候选 {candidate_idx} 提取直接注意力证据，程序终止。")
            return
        hypothesis, prompt = await generate_initial_hypothesis(
            open_router,
            sender_head,
            causal_examples,
            direct_attention_examples,

...
async def generate_initial_hypothesis(open_router, sender_head, causal_examples, explanation, output_dir):
    ...
    system_prompt = (
        ...
        "**--- Positional Structure Analysis (Critical for Attention Heads) ---**\n"
        "Attention heads are highly sensitive to positional patterns. You MUST analyze token positions using this notation:\n"
        "- **S1**: First occurrence of the Subject name (e.g., 'John' in 'John and Mary')\n"
        "- **S2**: Second occurrence of the Subject name (e.g., 'John' in 'Mary and John went... John gave')\n"
        "- **IO1**: First occurrence of the Indirect Object name\n"
        "- **IO2**: Second occurrence of the Indirect Object name\n"
        "- **S1+N / S2+N**: N tokens after S1/S2 position (e.g., S1+1 = token immediately after first subject)\n"
        "- **S1-N / S2-N**: N tokens before S1/S2 position\n"
        "- **END-N**: N tokens before the sentence end\n"
        "- **Structural positions**: Tokens at key syntactic boundaries (after commas, conjunctions, prepositions)\n\n"
        "When analyzing patterns, you must consider:\n"
        "1. **Absolute positions**: Where tokens appear in the sentence\n"
        "2. **Relative positions**: Distance from key entities (S1, S2, IO1, IO2)\n"
        "3. **Syntactic positions**: Roles in sentence structure (subject of clause, object of preposition, etc.)\n"
    )
    ...
    user_prompt = (
        ...
        "Please pay attention to the ':' after each tag, which means '[REASONING]:' is correct while '[REASONING] and [REASONING:] is incorrect'\n\n"
    )
...
async def predict_top_k_attenders_for_sentence(open_router, hypothesis, sid, sentence_data, sender_head, top_k, output_dir):
    ...
    prompt = (
        ...
        "**IMPORTANT: Positional Structure Analysis**\n"
        "You MUST think in terms of sentence structure and token positions:\n"
        "- **S1**: First occurrence of Subject name\n"
        "- **S2**: Second occurrence of Subject name (your attention source)\n"  
        "- **IO1, IO2**: First and second occurrence of Indirect Object name\n"
        "- **Relative positions**: S1+1, S2-1, END-2, etc.\n"
        "- **Syntactic positions**: Tokens after commas, conjunctions, prepositions\n\n"
    ...
    # 将最重要的token视为"increase"，次重要的视为"decrease"来复用这个函数
...
async def predict_for_single_sentence(open_router, hypothesis, sid, sentence_data, sender_head, top_k, output_dir, explanation):
    ...
    system_prompt = (
        ...
        "**--- Positional Structure Analysis System ---**\n"
        "You MUST analyze sentence structure using precise positional notation:\n"
        "- **S1, S2**: First and second occurrence of Subject name\n"
        "- **IO1, IO2**: First and second occurrence of Indirect Object name\n"
        "- **S1+N, S2+N**: N tokens after S1/S2 (e.g., S2+1 = token immediately after second subject)\n"
        "- **S1-N, S2-N**: N tokens before S1/S2\n"
        "- **END-N**: N tokens from sentence end\n"
        "- **Syntactic markers**: Positions relative to commas, conjunctions, prepositions\n\n"
    ...
    # 添加响应解析逻辑，以处理 [REASONING] 和 [PREDICTION]

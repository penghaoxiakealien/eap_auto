...
def evaluate_graph(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metrics: Union[Callable[[Tensor],Tensor], List[Callable[[Tensor], Tensor]]], quiet=False, intervention: Literal['patching', 'zero', 'mean','mean-positional']='patching', intervention_dataloader: Optional[DataLoader]=None, skip_clean:bool=True) -> Union[torch.Tensor, List[torch.Tensor]]:
    ...
    for clean, corrupted, label in dataloader:
        ...
        # 调试：打印 clean_tokens 和 corrupted_tokens
        #print("Clean tokens:", clean_tokens)
        #print("Corrupted tokens:", corrupted_tokens)
        
        # 检查输入数据是否为 None 或包含无效值
        if clean_tokens is None or corrupted_tokens is None:
            raise ValueError("Input tokens are None. Check tokenization.")
        if torch.isnan(clean_tokens).any() or torch.isnan(corrupted_tokens).any():
            raise ValueError("Input tokens contain NaN values.")

...
run_sih() {
  ...
  local head="$2"
  local mode="$3"
  local tag="${layer}.${head}_${mode}_${CAUSAL_DIRECTION}_rerun"
...
run_dth() {
  ...
  local tag="${head_tag}_to_9_6__9_9__10_0_${mode}_${CAUSAL_DIRECTION}_rerun"
...
# Missing / incomplete runs from the current summary:
run_sih 8 10 attention
run_dth 0 1 causal
run_dth 0 1 attention
run_dth 0 10 causal
run_dth 0 10 attention
run_dth 3 0 causal
run_dth 3 0 attention

echo "✅ IOI 0312 missing single-task decrease reruns finished."
